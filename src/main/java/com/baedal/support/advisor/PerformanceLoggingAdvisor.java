...
public class PerformanceLoggingAdvisor implements CallAdvisor {
    ...
    public ChatClientResponse adviseCall(ChatClientRequest request, CallAdvisorChain chain) {
        ...
        if (chatResponse != null && chatResponse.getMetadata() != null) {
            ...
            if (usage != null) {
                log.info("[LLM] elapsed={}ms inputTokens={} outputTokens={} totalTokens={}",
                        elapsed, usage.getPromptTokens(), usage.getCompletionTokens(), usage.getTotalTokens());
            } else {
                log.info("[LLM] elapsed={}ms (usage unavailable)", elapsed);
            }
        } else {
            log.info("[LLM] elapsed={}ms (response metadata unavailable)", elapsed);
        }
